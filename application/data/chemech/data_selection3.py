...
def convert_smiles_to_formula(smiles: str) -> str:
    ...
    for part in parts:
        ...
        if mol is None:
            ...
            canonical_parts.append(Chem.rdMolDescriptors.CalcMolFormula(mol))
...
def match_strategy(uspto_item: Dict[str, Any], mech_item: Dict[str, Any], strategy: int) -> bool:
    ...
    """
    if strategy == 9:
        return (convert_smiles_to_formula(uspto_item['rxts_cano']) == convert_smiles_to_formula(mech_item['rxts_cano_in_ori']) and
                convert_smiles_to_formula(uspto_item['prds_cano']) == convert_smiles_to_formula(mech_item['prds_cano_in_ori']))
    elif strategy == 10:
        return (convert_smiles_to_formula(uspto_item['rxts_cano']) == convert_smiles_to_formula(mech_item['rxts_cano_in_upd']) and
                convert_smiles_to_formula(uspto_item['prds_cano']) == convert_smiles_to_formula(mech_item['prds_cano_in_upd']))
        return False
...
def main(data_dir1: str, data_dir2: str, data_file: str, split_files: List[str]):
    ...
    for split in split_files:
        ...
        # 读取USPTO split数据
        uspto_file_path = os.path.join(data_dir2, f"uspto50k_{split}_rest.json")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir1", default="/mnt/e/DataSets/Chemistry/Chemech2/",
                        help="Directory containing mechanism data")
    parser.add_argument("--data_dir2", default="/mnt/e/DataSets/Chemistry/Chemech2/",
                        help="Directory containing USPTO50k split data")
    # 将data_selection生成的mech-USPTO_rest.json移到data_dir1下，并修改data_file为mech-USPTO_rest.json
    parser.add_argument("--data_file", default="mech-USPTO_rest.json", help="Mechanism data file name")
    parser.add_argument("--version", default="v3", help="Version")
