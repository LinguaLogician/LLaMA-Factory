...
def is_ascii(s):
    """检查字符串是否只包含ASCII字符（包括换行符、制表符等）"""
    for c in s:
        # 允许普通ASCII（0-127），以及特殊空白字符（如\n, \t, \r）
        if not (ord(c) < 128 or c in {'\n', '\t', '\r', 'Ġ'}):
            return False
    return True
...
def prune_vocab_and_remap(vocab):
    ...
    """
    # 先筛选出ASCII token和空格token，并按原始ID排序
    ascii_items = sorted([(k, v) for k, v in vocab.items() if is_ascii(k)], key=lambda x: x[1])

    # 额外识别并保留所有包含空格的token（即使不完全ASCII）
    space_items = [(k, v) for k, v in vocab.items() if 'Ġ' in k and not is_ascii(k)]
    combined_items = ascii_items + space_items
    combined_items.sort(key=lambda x: x[1])  # 重新按ID排序
    ...
    # 首先处理特殊token（确保ID不变）
    for token, old_id in combined_items:
    ...
    # 强制添加换行符（如果词表没有）
    if "\n" not in pruned_vocab:
        next_id = max(pruned_vocab.values()) + 1 if pruned_vocab else 0
        pruned_vocab["\n"] = next_id
        # 注意：这里没有 old_id，所以 old_to_new 无法映射，可能需要额外处理

    return pruned_vocab, old_to_new, special_tokens


def prune_merges(merges):
    """缩减merges列表，保留只包含ASCII字符或空格的merge"""
...
def main():
    ...
    print(f"移除的非ASCII token数量: {vocab_size - new_vocab_size:,}")
