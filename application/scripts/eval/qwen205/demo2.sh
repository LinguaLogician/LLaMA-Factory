...
MODEL_NAME="qwen205_moltrans_mit_mixed_nospace_lora_para3"
MODEL_PATH="/home/liangtao/Development/LLMSpace/LLaMA-Factory/output"
DATA_FILE="MIT_mixed.json"
CHECKPOINTS="ckpt460k"
FILE_PREFIX="mit_mixed_nospace_test_random1k"
...
process_checkpoint() {
    local checkpoint=$1
    local prediction_dir="${PREDICTION_BASE_DIR}/${checkpoint}"
    ...
    python "$SCORE_SCRIPT" \
        --prediction_file "${MODEL_NAME}_${checkpoint}" \
...
IFS=',' read -ra checkpoint_array <<< "$(echo "$CHECKPOINTS" | tr -d ' ')"
