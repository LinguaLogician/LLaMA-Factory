...
PARAM_PATH="examples/train_full/qwen205/nospace/qwen205_moltrans_stereo_separated_augm_nospace_full_para1.yaml"  # Example path
...
while true; do
    ...
    if [ "$AVAILABLE_MEM" -ge "$MIN_MEMORY" ]; then
        ...
        llamafactory-cli train "$PARAM_PATH" >> "$LOG_FILE_PATH" 2>&1
